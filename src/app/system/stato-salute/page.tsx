...
import {
  StatiSalute,
  useStatiSalute,
  useDeleteStatoSalute,
  useUpdateStatoSalute,
  useCreateStatoSalute
} from '@/hooks/useCrud';
...
const StatoSaluteTable = () => {
  ...
  const columns: DataTableColumn<StatiSalute>[] = [
    {
      id: 'nome',
      header: 'Nome',
      accessorKey: 'nome',
  ...
  const rowActions: DataTableAction<StatiSalute>[] = isSuperAdmin ? [
    ...
    },
    {
      ...
      label: 'Modifica',
      onClick: row => {
        setTitle('Modifica Stato di Salute');
        setSelectedRow(row);
        setDialogOpen(true);
      },
  ...
  const handleSave = (data: { newDescrizione?: string, newNome?: string }) => {
    ...
    if (selectedRow) {
      const payload: Partial<StatiSalute> = {
        id: selectedRow.id,
        descrizione: data.newDescrizione,
        nome: data.newNome
      };
      
      updateStatoSalute(payload, {
        onSuccess: () => {
          setAlert({
            show: true,
            type: 'success',
  ...
  const handelNewAction = () => {
    setTitle('Nuovo Stato di Salute');
    setSelectedRow(null);
    setDialogOpen(true);
  };
    ...
    <>
      ...
      >
        <DataTable
          ...
          loading={isLoading}
          searchKey="nome"
          searchPlaceholder="Cerca stato di salute..."
          emptyMessage="Nessun stato di salute trovato"
...
export default StatoSaluteTable;
