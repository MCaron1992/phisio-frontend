...
import {
  ...
  useUpdateRegioneAnatomica,
  useCreateRegioneAnatomica
} from '@/hooks/useCrud';
...
const RegioneAnatomicaTable = () => {
  ...
  const rowActions: DataTableAction<RegioniAnatomicha>[] = isSuperAdmin ? [
    ...
    },
    {
      ...
      label: 'Modifica',
      onClick: row => {
        setTitle('Modifica Regione Anatomica');
        setSelectedRow(row);
        setDialogOpen(true);
      },
  ...
  const handleSave = (data: { newDescrizione?: string, newNome?: string }) => {
    ...
    if (selectedRow) {
      const payload: Partial<RegioniAnatomicha> = {
        id: selectedRow.id,
        descrizione: data.newDescrizione,
        nome: data.newNome 
      };

      updateRegioneAnatomica(payload, {
        onSuccess: () => {
          setAlert({
            show: true,
            type: 'success',
      ...
      createRegioneAnatomica(payload, {
        onSuccess: () => {
          setAlert({
            show: true,
            type: 'success',
  ...
  const handelNewAction = () => {
    setTitle('Nuova Regione Anatomica');
    setSelectedRow(null);
    setDialogOpen(true);
  };
...
export default RegioneAnatomicaTable;
