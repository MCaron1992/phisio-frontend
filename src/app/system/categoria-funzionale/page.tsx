...
const CategoriaFunzionaleTable = () => {
  ...
  const [selectedRow, setSelectedRow] = useState<CategoriaFunzionale | null>(null);
  ...
  const handleSave = (data: { newDescrizione: string }) => {
    setLoading(true);
    updateCategoriaFunzionale(data, {
      onSuccess: () => {
        setAlert({
          show: true,
          type: 'success',
  ...
  return (
    <>
      ...
      />
      <DeleteConfirmDialog
        onConfirm={() => deleteCategoriaFunzionale({ id: selectedRow?.id! })}
