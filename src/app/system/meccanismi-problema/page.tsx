...
const MeccanismiProblemaTable = () => {
  ...
  const { mutate: createMeccanismoProblema } = useCreateMeccanismoProblema();
  ...
  const rowActions: DataTableAction<MeccanismoProblema>[] = isSuperAdmin ? [
    ...
    },
    {
      ...
      label: 'Modifica',
      onClick: row => {
        setTitle('Modifica Meccanismo Problema');
        setSelectedRow(row);
        setDialogOpen(true);
      },
  ...
  const handleSave = (data: { newDescrizione?: string, newNome?: string }) => {
    ...
    if (selectedRow) {
      const payload: Partial<MeccanismoProblema> = {
        id: selectedRow.id,
        descrizione: data.newDescrizione,
        nome: data.newNome
      };

      updateMeccanismoProblema(payload, {
        onSuccess: () => {
          setAlert({
            show: true,
            type: 'success',
      ...
      createMeccanismoProblema(payload, {
        onSuccess: () => {
          setAlert({
            show: true,
            type: 'success',
  ...
  const handelNewAction = () => {
    setTitle('Nuovo Meccanismo Problema');
    setSelectedRow(null);
    setDialogOpen(true);
  };
    ...
    <>
      ...
      >
        <DataTable
          ...
          emptyMessage="Nessun meccanismo trovato"
...
export default MeccanismiProblemaTable;
