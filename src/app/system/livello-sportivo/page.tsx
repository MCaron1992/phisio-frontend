...
const LivelloSportivoTable = () => {
  ...
  const { mutate: createLivelloSportivo } = useCreateLivelloSportivo();
  ...
  const rowActions: DataTableAction<LivelloSportivi>[] = isSuperAdmin ? [
    {
      id: 'view',
      label: 'Visualizza',
  ...
  const handleSave = (data: { newDescrizione?: string, newNome?: string }) => {
    ...
    if (selectedRow) {
      const payload: Partial<LivelloSportivi> = {
        id: selectedRow.id,
        descrizione: data.newDescrizione,
        nome: data.newNome,
      };

      updateLivelloSportivo(payload, {
        onSuccess: () => {
          setAlert({
            show: true,
            type: 'success',
