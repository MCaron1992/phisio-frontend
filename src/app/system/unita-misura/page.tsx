...
import {
  ...
  useUpdateUnitaMisura,
  useCreateUnitaMisura,
} from '@/hooks/useCrud';
...
const UnitaMisuraTable = () => {
  ...
  const rowActions: DataTableAction<UnitaMisura>[] = [
    ...
    },
    {
      ...
      label: 'Modifica',
      onClick: row => {
        setTitle('Modifica Unità di Misura');
        setSelectedRow(row);
        setDialogOpen(true);
      },
  ...
  const handleSave = (data: { newDescrizione?: string, newNome?: string }) => {
    ...
    if (selectedRow) {
      const payload: Partial<UnitaMisura> = {
        id: selectedRow.id,
        descrizione: data.newDescrizione,
        nome: data.newNome 
      };

      updateUnitaMisura(payload, {
        onSuccess: () => {
          setAlert({
            show: true,
            type: 'success',
  ...
  const handelNewAction = () => {
    setTitle('Nuova Unità di Misura');
    setSelectedRow(null);
    setDialogOpen(true);
  };
    ...
    <>
      ...
      >
        <DataTable
          ...
          loading={isLoading}
          searchKey="nome"
          searchPlaceholder="Cerca unità di misura..."
          emptyMessage="Nessuna unità di misura trovata"
...
export default UnitaMisuraTable;
